...
foreach ($vm in $VMsConfigContent.vms) {
	...
	Invoke-Command -ComputerName $IpAddress -Credential $VHDImageCredentials -ScriptBlock {
		param ($ComputerName, $DomainCredentials)
		$ErrorActionPreference = 'SilentlyContinue'
		if (-not $( Get-ADComputer -Identity "$ComputerName")) {
			return $false
		}
		else {
			Remove-Computer -UnjoinDomaincredential $DomainCredentials -PassThru -Verbose -Restart
		}
		$ErrorActionPreference = 'Continue'
	} -ArgumentList $ComputerName, $DomainCredentials
...
.\Roles\RemoveVMs\DSC\DSCRemoveVMs.ps1 -ConfPath .\Hosts\$($SelectForm.HostsConfig) -RoleConfig .\Roles\DeployWACHA\Config\$($SelectForm.VMsConfig)
Start-DscConfiguration -Path ".\Roles\RemoveVMs\DSC\temp\" -Verbose -Wait -Force
Get-ChildItem ".\Roles\RemoveVMs\DSC\temp\" -Include *.mof -Recurse | Remove-Item
